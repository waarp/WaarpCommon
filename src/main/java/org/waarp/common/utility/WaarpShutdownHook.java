...
public abstract class WaarpShutdownHook extends Thread {
    ...
    /**
     * Sun property pointing the main class and its arguments. Might not be defined on non Hotspot
     * VM implementations.
     */
    ...
    /**
     * Restart the application using the preset applArgs and computing the jvmArgs. execute the
     * command in a shutdown hook, to be sure that all the resources have been disposed before
        ...
        if (shouldRestart) {
            try {
                ...
                if (DetectionUtils.isWindows()) {
                    cmd = new StringBuilder("\"").append(java).append("\" ")
                            .append(vmArgsOneLine);
                } else {
                    cmd = new StringBuilder(java).append(" ").append(vmArgsOneLine);
                }
